...
public:
    ...
    Q_INVOKABLE void remove_node(TimelineNode* what);

    Q_INVOKABLE QQuickItem* fetch_node(QPointF where);

    Q_INVOKABLE void add_chain(QPointF where);
    Q_INVOKABLE void remove_chain(TimelineChain* what);

    Q_INVOKABLE QQuickItem* fetch_chain(QPointF where);
...
private:
    QPolygonF m_polygon{};
    std::set<TimelineNode*> m_nodes{};
    std::set<TimelineChain*> m_chains{};
};
